...
支持EXPIRE自动过期，无需手动清理
- PostgreSQL
与FastAPI集成成熟，同时相较MYSQL也作为一个热门方向在发展，社区活跃度高
...
| 技术 | 版本 | 用途 |
|------|------|------|
| React | 19.2.6 | 构建用户界面的核心框架 |
| Vite | 8.0.11 | 极速的开发服务器与构建工具 |
| TypeScript | 6.0.3 | 静态类型检查，提升代码健壮性 |
| Tailwind CSS | 3.4.19 | 实用优先的 CSS 框架，快速构建自定义界面 |
| React Router DOM | 7.15.0 | 客户端路由，管理多页面与 URL 参数 |
| Axios | 1.16.0 | 基于 Promise 的 HTTP 客户端，与后端 API 交互 |
| Zustand | 5.0.13 | 轻量级状态管理，存储全局用户状态 |

#### 选型理由

- **React**  
  生态丰富、组件化开发高效、社区活跃，适合构建复杂交互的单页应用，且与桌面端 Electron 技术栈一致，方便代码复用。

- **Vite**  
  开发服务器冷启动极快，热更新（HMR）性能优异；基于 Rollup 的打包配置简洁，开箱即用支持 TypeScript 和 React。

- **TypeScript**  
  静态类型系统可在编码阶段发现潜在错误，配合 VSCode 提供优秀的智能提示，降低维护成本，提升代码质量。

- **Tailwind CSS**  
  无需离开 HTML 即可通过组合原子类快速构建布局，避免编写大量自定义 CSS，样式一致性高，且打包后体积小（可自动移除未使用的样式）。

- **React Router DOM**  
  官方推荐的路由库，支持声明式路由配置、嵌套路由、URL 参数读取（如注册链接中的 token），完全满足单页应用需求。

- **Axios**  
  语法简洁，支持请求/响应拦截器（可统一添加 Token、处理错误），自动转换 JSON，超时设置，并能在 Node.js（Electron）中复用同一套请求逻辑。

- **Zustand**  
  极简的全局状态管理方案，对比 Redux 代码量少、学习曲线平缓，无需 Provider 包裹，适合管理用户登录状态、主题等全局数据。